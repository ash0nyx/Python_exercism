...
def round_scores(student_scores):
    ...
    return [round(s) for s in student_scores if type(student_scores) == list]
...
def letter_grades(highest):
    ...
    return [x for x in range(41, highest,round((highest-41)/4))]
...
def perfect_score(student_info):
    ...
    return next(([x,y] for [x,y] in student_info if y == 100), [])